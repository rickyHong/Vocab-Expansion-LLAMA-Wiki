...
### Step 2: 准备模型权重
首先运行以下命令将下载后的lora权重放到loras文件夹下
```bash
...
```
然后将HuggingFace格式的llama-7B模型文件放到models文件夹下
```bash
...
```
最后复制lora权重的tokenizer到models/llama-7b-hf下(webui默认从`./models`下加载tokenizer.model,因此需使用扩展中文词表后的tokenizer.model)
```bash
...
```
### Step 3: 修改/modules/LoRA.py文件
运行前需在`PeftModel.from_pretrained`方法之前添加一行代码修改原始llama的embed_size
```bash
