...
更多关于中文词表扩充的动机，可参考[FAQ](./常见问题#问题4为什么要扩充词表直接在原版llama上用中文预训练不行吗)。如果欲了解扩充词表的具体方法，或者使用自己的词表对LLaMA tokenizer进行扩充，我们提供了代码[merge_tokenizers.py](./scripts/merge_tokenizer/merge_tokenizers.py)供参考。该脚本运行方式如下：
