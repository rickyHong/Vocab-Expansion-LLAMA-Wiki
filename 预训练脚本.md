我们这里给出Pre-training Stage 2的训练脚本。
Pre-training Stage 1的训练效率较低，模型收敛速度较慢，如果不是有特别充裕的时间和计算资源，建议跳过该阶段。

预训练脚本[scripts/run_clm_pt_with_peft.py](https://github.com/ymcui/Chinese-LLaMA-Alpaca/blob/main/scripts/run_clm_pt_with_peft.py)的启动命令如下（部分变量的值需由用户指定）：
