...
节省显存小提示：
* 如果机器的显存比较紧张，可以删去脚本中的`--modules_to_save ${modules_to_save} \`, 即不训练embed_tokens和lm_head（这两部分参数量较大），只训练LoRA参数，以节省显存（建议在Chinese-LLaMA的基础上实验，而非从头预训练时就不训练embed_tokens和lm_head）
