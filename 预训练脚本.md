...
```bash
torchrun --nnodes ${num_nodes} --nproc_per_node ${num_gpu_per_node} --node_rank ${node_rank} --master_adder ${master_addr} --master_port ${master_port} run_clm_pt_with_peft.py ...
```
