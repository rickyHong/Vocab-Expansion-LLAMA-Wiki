进入项目的scripts目录，指令精调脚本[scripts/run_clm_sft_with_peft.py](https://github.com/ymcui/Chinese-LLaMA-Alpaca/blob/main/scripts/run_clm_sft_with_peft.py)（**没上传，还没整理完，晚些时候再来看**）的启动命令如下（部分变量的值需由用户指定）：
